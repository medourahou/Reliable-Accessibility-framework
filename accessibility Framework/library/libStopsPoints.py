...
#         print("for 1...")
        ...
        if(nameC[:-4] in gtfsDB.list_collection_names()): gtfsDB[nameC[:-4]].delete_many({'city':city})
...
#                 print("for 3...")
                ...
                if nameCSVFile in archive.namelist():
                    ...
                    print('{2} -> ({0},{1})'.format(len(res),len(list(collGtfs.find({'city':city}))),nameCSVFile))
...
def removingStopsNoConnections(gtfsDB, city):
    ...
    count = 0
    nb=0
    stops = list(gtfsDB['stops'].find({'city':city}).sort([('_id', pym.ASCENDING)]))
    for stop in range(0, len(stops)):
        #print("number of stops in this step:",len(stop))
        countPStart = gtfsDB['connections'].count_documents({'city':city,'file':stops[stop]['file'],'pStart':stops[stop]['stop_id']})
        # countPStart = gtfsDB['connections'].find({'city':city,'file':stops[stop]['file'],'pStart':stops[stop]['stop_id']}).count()
        countPEnd = gtfsDB['connections'].count_documents({'city':city,'file':stops[stop]['file'],'pEnd':stops[stop]['stop_id']})
        # countPEnd = gtfsDB['connections'].find({'city':city,'file':stops[stop]['file'],'pEnd':stops[stop]['stop_id']}).count()
        if countPStart == 0 and countPEnd == 0:
            gtfsDB['stops'].delete_one({'_id':stops[stop]['_id']})
            #print("delete one stop")
            if stops[stop]['file'] in count_rem:
                count_rem[stops[stop]['file']] += 1
            else:
                count_rem[stops[stop]['file']] = 1
        else:
            pass
...
def setPosField(gtfsDB, city):
    ...
    for stop in range(0,len(stops)):
      try:
        nb+=1
        #print("number of stops ",nb)
        gtfsDB['stops'].update_one({'_id':stops[stop]['_id']},
                                   {'$set':
                                       {
                                        'pos':pos,
                                        'point':
                                        { 'type': "Point" , 'coordinates': [float(stops[stop]['stop_lon']), float(stops[stop]['stop_lat'])]}
                                       }
                                   });
        pos += 1
        print ('{0}'.format(pos),end="\r")
      except:
        print("CursorNotFound: Deleting the problematic stop.")
        gtfsDB['stops'].delete_one({'_id': stops[stop]['_id']})
...
def computeNeigh(gtfsDB, urlServerOsrm, distanceS, tS, city):
    ...
    avgNeightS = 0
    avgNeightP = 0
    tot = gtfsDB['stops'].count_documents({'stopN':{'$exists':False},'city':city}) + gtfsDB['points'].count_documents({'stopN':{'$exists':False},'city':city})
    #tot = gtfsDB['stops'].find({'city':city}).count() + gtfsDB['points'].find({'city':city}).count()
    count = 0
    avgT = 0
    total_stops= list(gtfsDB['stops'].find({'stopN':{'$exists':False},'city':city}, no_cursor_timeout=True))
    ...
    total_points = list(gtfsDB['points'].find({'pointN':{'$exists':False},'city':city}, no_cursor_timeout=True))
...
def listPointsStopsN(gtfsDB, city):
    maxStop = gtfsDB['stops'].count_documents({'city':city})
    maxPoint = gtfsDB['points'].count_documents({'city':city})
    maxRow = -1
